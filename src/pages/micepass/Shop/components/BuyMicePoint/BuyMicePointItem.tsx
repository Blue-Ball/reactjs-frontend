...
interface Props {
  micePoint: MicePointTypeProps;
}

const BuyMicePointItem: FC<Props> = (props) => {
  const classes = useStyles();

  const { micePoint } = props;

  return (
    <Box className={classes.pointItem}>
      <Grid container>
        <Grid item xs={12} sm={6} md={4} className={classes.titleCell}>
          <Typography variant="body1">{micePoint.title}</Typography>
        </Grid>
        <Grid item xs={12} sm={6} md={4} className={classes.valueCell}>
          <Typography variant="body1">{`TWD ${micePoint.value}`}</Typography>
        </Grid>
